...
class EDA_Engine:
    ...
    def overview(self) -> None:
        """
        Function to print an overview of the DataFrame.
        It will print the shape, data types, first few rows, and other relevant information.
        :return: None
        """
    ...
    def missing_values(self):
        ...
        nr_rows_with_nan = self.data_df.isnull().any(axis=1).sum()
        prctg_rows_with_nan = nr_rows_with_nan/self.data_df.shape[0]*100
        print(f"Rows with at least one NaN --- Nr. Rows: {nr_rows_with_nan:.2f} --- Percentage: {prctg_rows_with_nan:.2f}%\n")
        ...
        for i, col_name in enumerate(self.data_df.columns):
            ...
            print(
                f"{i} Column: {col_name} --- Missing values: {missing_vals:.2f} --- Percentage: {missing_prctg:.2f}%"
            )
    ...
    def numerical_values(self):
        ...
        It will print the measures of statistical dispersion each numerical column.
        ...
        for i, col_name in enumerate(self.data_df.columns):
            if not is_object_dtype(self.data_df[col_name]):
                print(
                    f"{i} Column: {col_name} --- Nr.Uniques: {len(self.data_df[col_name].unique())} --- Min.: {self.data_df[col_name].min():.2f} --- Max.: {self.data_df[col_name].max():.2f} --- Mean: {self.data_df[col_name].mean():.2f} --- Median: {self.data_df[col_name].median():.2f} --- StdDev: {self.data_df[col_name].std():.2f}"
                )
        print("\n")
    ...
    def categorical_values(self, plot_ok: bool = False) -> None:
        ...
        print("Describe the categorical columns\n")
        categrical_cols = self.data_df.select_dtypes(
        ...
        for col_name in categrical_cols:
            ...
            for unique_val in unique_vals_ls:
                if pd.isna(unique_val):
                    prctg_occ = (
                        self.data_df[col_name].isna().sum()
                    ...
                    prctg_occ = (
                        ...
                        .str.fullmatch(unique_val, case=True)
                        .sum()
                        / self.data_df.shape[0]
                        * 100
                    )
            ...
            if plot_ok:
                ...
                patches, texts, pcts = ax.pie(
                    x=unique_vals_prctg_ls, labels=unique_vals_ls, autopct="%.0f%%"
                )
